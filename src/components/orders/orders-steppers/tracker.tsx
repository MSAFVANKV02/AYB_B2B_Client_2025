...
const Tracker = () => {
  ...
  return (
    <div className="flex justify-between items-center w-full px-4 py-6 bg-white rounded-md">
      {steps.map((step, index) => (
        <div
          key={step.id}
          // className={`
          //   relative flex flex-col gap-3 items-center text-center w-full
          //   before:content-[''] before:absolute before:top-1/2 before:left-0 before:h-1 before:w-1/2 ${index === currentStep ||index < currentStep ?"after:bg-[#12B76A] before:bg-[#12B76A]":"after:bg-[#D0D5DD] before:bg-[#D0D5DD]"}
        ...
        >
          ...
          {/* Dot */}
          <div
            className={`z-10 w-4 h-4 rounded-full ${index === currentStep || index < currentStep ? "bg-[#12B76A]" : "bg-[#D0D5DD]"} `}
          ></div>
